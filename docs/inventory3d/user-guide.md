...
Select a site, click on the Record`s Details Tool 

and the list of objects opens: 
...
and azimuth values will be displayed between the starting point and the mouse cursor. 
 ...
 , left-click on the starting point, then move the mouse over the map - distance 

Link to database 

Map and database are functionally connected. Select the info tool 
map (here: Abava), and a popup window opens:  

. Then click on a site in the 
...
The popup window comprises links to the site (

) and to the attachments file browser (
...
Display local weather conditions by clicking on 
and then on a custom position on the map. A 
...
information to the database. 
 ...
 to open the Inventory3D page for the new object, edit the attributes and save the 
...
select  the  record,  press  the  button 
interface (method A) 

  and  create  the  diagram  using  the  graphical  drawing 
...
“Diagram_items“ and “Diagram_links“ (method B) 
  ...
  and  create  the  diagram  records  in  the  tables 
...
go to the table “Diagrams“, select the diagram record, and press the button 
